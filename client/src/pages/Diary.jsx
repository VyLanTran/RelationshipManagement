    ...
    useEffect(() => {
        const getFetch = async () => {
            ...
            if (res_diary.ok) {
                ...
                setGroupDiaries(
                    data_diary.filter((diary) => diary['group'] == null)
                )
            }
        }
        if (user) {
    ...
    const deleteDiary = async (event, diary) => {
        ...
        try {
            const res = await axios.delete(
                `http://localhost:3001/diary/${diary._id}`,
                authHeader
            )
            ...
            <div className="flex flex-row">
                    ...
                    <div className="h-[88vh] bg-[#FFF] rounded-[10px] p-[15px] my-[5px]">
                        ...
                        <div className="mt-[10px] h-[77vh] overflow-x-auto">
                            {groupDiaries.map((diary) => (
                                <button
                                    className="group/item w-full h-[70px] bg-[#EEE] mb-[5px] rounded-[10px] flex flex-row items-center justify-between hover:bg-[#ffdc8b] focus:bg-[#FFB302]"
                                    onClick={() => {
                                ...
                                >
                                    ...
                                    <div className="text-white group/edit invisible hover:text-red-500 group-hover/item:visible">
                                        <button
                                            className="group-hover/edit:text-gray mr-[10px] pt-[5px]"
                                            onClick={(event) =>
                                                deleteDiary(event, diary)
                                            }
                                        >
                                            <Trash2
                                                size={24}
                ...
                <div className="w-full lg:w-3/4 mx-[5px] h-[88vh]">
                    <div class="overflow-x-auto flex justify-center items-center h-[55px] block bg-[#FFF] rounded-[10px] my-[5px]">
                        ...
                        {groups.map((group) => (
                            <button
                                className="text-xl bg-[#EEE] rounded-[5px] h-[45px] p-[8px] m-[3px] hover:cursor-pointer hover:bg-[#ffdc8b] focus:bg-[#FFB302] flex-none"
                                onClick={() => {
                                    setCurrentGroup(group)
                                    setGroupDiaries(
                                        diaries.filter(
                                            (diary) =>
                                                diary['group'] == group['_id']
                                        )
                                    )
                                }}
                            >
                    ...
                    {currentDiary ? (
                        <div className="bg-[#FFF] rounded-[10px] p-[10px] h-[79.6vh]">
                            <div className="flex flex-row items-center">
                                ...
                                />
                                <button
                                    className="text-slate-400 hover:text-black mb-[5px]"
